...
real = ti.f32
ti.init(default_fp=real, arch=ti.cuda, flatten_if=True, device_memory_GB=1)
...
num_segs = 4 # random.randint(1,max_segs)
seg_heights = ti.field(dtype=real, shape=num_segs, needs_grad=True)
for i in range(num_segs):
    rand_height = np.random.uniform(min_segh, max_segh)
    seg_heights[i] = rand_height
...
@ti.kernel
def update_seg_heights(arr: ti.types.ndarray(dtype=ti.f32)):
    for i in range(num_segs):
        seg_heights[i] += arr[i]
...
def compute_loss():
    ...
    seg_loss = 0
    seg_loss += seg_heights[0]-seg_heights[1]
    seg_loss += seg_heights[1]-seg_heights[2]
    seg_loss += seg_heights[2]-seg_heights[3]
    loss[None] -= seg_loss
...
def snake(scene, seg_hts_input):
    scene.set_offset(0.1, head_height-0.05)
    for i in range(num_segs):
        scene.add_rect((i)*seg_width, (head_height-seg_hts_input[i])/2, seg_width, seg_hts_input[i], i)
    scene.add_rect(num_segs*seg_width, 0.0, seg_width, head_height, -1)  # head
    scene.set_n_actuators(num_segs)

def reset_scene():
    global x, v, actuator_id, particle_type, n_particles, n_solid_particles
    
    scene = Scene()
    snake(scene, seg_heights)  # Update with new heights
    scene.finalize()
    actuator_id = ti.field(ti.i32)
    particle_type = ti.field(ti.i32)
    ti.root.dense(ti.i, n_particles).place(actuator_id, particle_type)
...
def visualize(s, folder):
    ...
    for i in range(n_particles):
        ...
        colors[i] = color
    gui.circles(pos=particles, radius=1.5)
    gui.line((0.05, 0.02), (0.95, 0.02), radius=3, color=0x0)

    os.makedirs(folder, exist_ok=True)
    gui.show(f'{folder}/{s:04d}.png')

# initialize the fields I want to reset every loop
ti.root.dense(ti.i, n_particles).place(actuator_id, particle_type)
...
def main():
    ...
    for iter in range(options.iters):
        ...
        # update segment heights
        sh = []
        for i in range(num_segs):
            rand_height = np.random.uniform(min_segh, max_segh)
            print("seg_ht grad: ", seg_heights.grad[i])
            sh.append(-10*learning_rate*seg_heights.grad[i])     # I added - optimize segment heights
        arr = np.array(sh, dtype=np.float32)
        update_seg_heights(arr)
        #
